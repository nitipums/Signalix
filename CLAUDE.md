...
- **Classifier changes** (anything in `analyzer.classify_stage` / `classify_sub_stage`) MUST keep `python3 scripts/audit_set100.py` exit-code 0 before shipping. The audit runs the FSM against every SET100 member with live yfinance data + 12 rule-of-thumb sanity checks against price-action expectations (parent-stage match, sub-stage definition match, 60d-return / pct-from-high / peak-recency consistency). Any block-severity flag = 1 means a category of stocks is mis-classified and needs investigation. The audit exists because spot-checking 4 stocks isn't enough — KTC/SINGER/COM7/ERW were all stage-3-misfires that only surfaced via full-universe iteration. Update the audit's expectations alongside any deliberate semantic change to the classifier.
